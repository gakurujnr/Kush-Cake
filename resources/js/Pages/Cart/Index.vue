      ...
      <form class="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
        <section aria-labelledby="cart-heading" class="lg:col-span-7">
          ...
          <ul role="list" class="divide-y divide-gray-200 border-b border-t border-gray-200">
            <li v-for="(orderItem, orderItemsIdx) in order.order_items" :key="orderItem.id" class="flex py-6 sm:py-10">
              <div class="flex-shrink-0">
                <img :src="orderItem.product.image.url" :alt="orderItem.product.name" class="h-24 w-24 rounded-md object-cover object-center sm:h-48 sm:w-48" />
              </div>
                ...
                <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                  <div>
                    <div class="flex justify-between">
                      <h3 class="text-sm">
                        <a :href="orderItem.product.id" class="font-medium text-gray-700 hover:text-gray-800">{{ orderItem.product.name }}</a>
                      </h3>
                    </div>
                  ...
                  <div class="mt-4 sm:mt-0 sm:pr-9">
                    <label :for="`quantity-${orderItemsIdx}`" class="sr-only">Quantity, {{ orderItem.product.name }}</label>
