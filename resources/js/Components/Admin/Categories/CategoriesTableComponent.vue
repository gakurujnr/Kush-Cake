    ...
    <table class="min-w-full table-auto">
      ...
      <tbody>
        <tr v-for="(category, key) in categories" :key="key"
          :class="`${key === categories.length - 1 ? '' : 'border-b border-stroke dark:border-strokedark'}`">
          ...
          <td class="p-2.5 text-center xl:p-5">
            <p class="text-black dark:text-white">{{ useDateFormat(category.created_at,'Do MMMM YYYY HH:mm:ss') }}</p>
          </td>
        </tr>
      </tbody>
    </table>
